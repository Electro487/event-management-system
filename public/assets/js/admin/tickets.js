/**
 * Admin Tickets Management JS
 * Filters only for 'Concert' category
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadTickets(searchParam || '');
    initFilters();
});

async function loadTickets(initialSearch = '') {
    if (!window.emsApi) return;

    try {
        const res = await window.emsApi.apiFetch('/api/v1/bookings');
        if (res.success) {
            // Filter only concert bookings
            allBookings = (res.data.items || []).filter(b => {
...
function renderTable() {
    ...
    if (!filteredBookings.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="no-data">No tickets found matching your criteria.</td></tr>';
        renderPagination(0);
        return;
    }
    ...
    tbody.innerHTML = pageItems.map(b => {
        const eSnap = b.event_snapshot ? JSON.parse(b.event_snapshot) : null;
        const dispTitle = eSnap?.title || b.event_title || 'Untitled Concert';
        ...
        const avatarHtml = b.client_profile_pic 
            ? `<img src="${b.client_profile_pic}" alt="Attendee" class="client-avatar-img" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover; flex-shrink: 0; box-shadow: 0 2px 5px rgba(0,0,0,0.05); margin-right: 12px;">`
            ...
            <tr class="booking-row">
                ...
                <td style="text-align:right; padding-right:24px;">
                    <div class="action-cell">
                        <a href="/EventManagementSystem/public/admin/bookings/view?id=${b.id}&source=tickets" class="btn-view">View Details</a>
                    </div>
                </td>
            </tr>
        `;
...
async function approveTicket(id) {
    ...
    } catch (err) { console.error('Error approving ticket:', err); }
}
